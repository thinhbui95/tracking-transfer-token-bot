...
pub async fn get_detail_tx(rpc_url: String, ws_url: String, mint: String, decimal: u8, program_token_id: Pubkey) -> Result<(), Box<dyn Error + Send + Sync>> {
    ...
    while let Ok(logs) = receiver.recv() {
        ...
        if !signature.is_empty() && logs.value.err.is_none() {
            ...
            tokio::spawn(async move {
                ...
                if let Err(e) = get_info_transaction_with_retry(client, rpc_selector, &mint, decimal, &program_token_id, &sig_clone, verified_accounts).await {
...
async fn get_info_transaction_with_retry(
    ...
    for attempt in 0..=max_retries {
        ...
        match timeout(
            request_timeout,
            get_info_transaction(current_client, filter_account, decimal, token_program_id, tx_signature, verified_accounts.clone())
        ).await {
            Ok(Ok(())) => return Ok(()),
...
async fn get_info_transaction(
    ...
    let inner_instructions = meta.inner_instructions.ok_or("No inner instructions")?;
        ...
        for ix in instruction_group.instructions {
            if let UiInstruction::Parsed(UiParsedInstruction::Parsed(parsed_info)) = ix {
                ...
                if let (Some(source), Some(destination), Some(amount_str)) = (source, destination, amount_str) {
                    ...
                    println!("Transfer: {} -> {} | Value: {} at {}\n", 
                             source, destination, amount_value, tx_signature);
                }
            }
        }
    }
...
mod tests {
    ...
    async fn test_get_detail_tx_live() {
        ...
        let monitor_task = tokio::spawn(async move {
            if let Err(e) = get_detail_tx(rpc_url, ws_url, mint, decimal, token_program_id).await {
    ...
    async fn test_get_info_transaction() {
        let rpc_url = "https://go.getblock.asia/cfaef04d17084aa99dc018ffadb2bee2".to_string();
        let mint = "SarosY6Vscao718M4A778z4CGtvcwcGef5M9MEH1LGL".to_string();
        let token_program_id = Pubkey::from_str("TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA").unwrap();
        let decimal = 6;
        let tx_signature = "5DNyL1gmgfqDRyKd9u9XZJq74ssBFxhDAcihn4m9rgFR1Xc4YgjBBF4AL7JsnLEmUX6Ue65Pm1dD5gXZUzq2fucB".to_string();
        ...
        let result = get_info_transaction(client, &mint, decimal, &token_program_id, &tx_signature, verified_accounts).await;
        assert!(result.is_ok());
    }
}